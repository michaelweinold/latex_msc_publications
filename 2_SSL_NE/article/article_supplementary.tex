...
% formatting
\input{auxiliary/preamble/3_preamble_formatting_listings}
...
% https://tex.stackexchange.com/a/712523

\newcounter{suppnotecounter}
\newcommand{\suppnote}[1]{%
  \stepcounter{suppnotecounter}%
  \subsection*{\large\bfseries\filright Supplementary Note \arabic{suppnotecounter}: \\ #1}%
  \addcontentsline{toc}{subsection}{#1}%
  \label{suppnote:#1}
}

\newcounter{suppdisccounter}
\newcommand{\suppdisc}[1]{%
  \stepcounter{suppdisccounter}%
  \subsection*{\large\bfseries\filright Supplementary Discussion \arabic{suppdisccounter}: \\ #1}%
  \addcontentsline{toc}{subsection}{#1}%
}

\newcounter{suppmethcounter}
\newcommand{\suppmeth}[1]{%
  \stepcounter{suppmethcounter}%
  \subsection*{\large\bfseries\filright Supplementary Method \arabic{suppmethcounter}: \\ #1}%
  \addcontentsline{toc}{subsection}{#1}%
}

\newcounter{supprefcounter}
\newcommand{\suppref}[1]{%
  \stepcounter{supprefcounter}%
  \subsection*{\large\bfseries\filright Supplementary References \arabic{supprefcounter}: \\ #1}%
  \addcontentsline{toc}{subsection}{#1}%
}
...
\suppnote{Previous Literature on Technological Progress in LEDs}
\label{sec:prev_lit}
...
\suppnote{Manufacturing Process Steps}
...
\suppnote{Historical Progress in Chip Architectures}
...
\clearpage
\suppnote{Historical Progress in Sub-Efficiencies}
\label{sec:subeff_progress}

\begin{figure}[H]
\centering
\includegraphics[width=15.5cm]{./figures/subefficiencies_progress.pdf}
\caption{\textbf{Historical improvements in LED sub-efficiencies.} Panels A1-A2 show data for key metrics used to calculate forward voltage efficiency $\eta_{Vf}$ and droop efficiency $\eta_{droop}$. Panels B1-B2 show data for internal quantum efficiency $\eta_{IQ}$ and light extraction efficiency $\eta_{LE}$. Source: own synthesis of data from the list of sources provided in \cref{sec:sources}. For an illustration of the different chip architectures shown, compare \cref{sec:chip_architecture}. Data on average device performance adapted from U.S. Department of Energy (DOE) Reports \cite{doe_ssl_multiyear_2007}\cite{doe_ssl_multiyear_2008}\cite{doe_ssl_multiyear_2013}\cite{doe_ssl_rnd_2016}\cite{doe_ssl_rnd_2018} and an Osram company report \cite{beale_leds_2015}.}
\label{fgr:subeff}
\vspace{-20mm}
\end{figure}
...
\clearpage
\suppnote{Non-Phosphor-Related LED Innovations}
...
\clearpage
\suppnote{Phosphor-Related LED Innovations}
...
\suppnote{Spectral Data for Phosphors}
...
\clearpage
\suppref{Full List of Sources for Figures}
\label{sec:sources}
...
\suppmeth{Domain Definition}
\label{sec:domain}

\textit{Elsevier Scopus}\footnote{\url{https://www.scopus.com/search/}} advanced search query:
...
\suppmeth{Metrics for Quantifying Technological Progress in LEDs}
...
\clearpage
\suppmeth{Device Performance Metrics}
...
\clearpage
\suppmeth{Consumer Experience Metrics}
...
\suppmeth{Background of Interviewed Experts}
...
\clearpage
\suppmeth{Cost Model Structure and Limitations}
...
\suppmeth{Contribution of Variables}
...
\addcontentsline{toc}{section}{Supplementary Discussion}

\suppdisc{Cost Model Sensitivity Analysis}
...
\clearpage
\suppdisc{Cost Data Comparison with Industry Projections}
